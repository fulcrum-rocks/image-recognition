...
class ConfigParser(object):
    ...
    def create_detector(self, model):
        d = YoloDetector(
            model, self._model_config["anchors"], net_size=self._model_config["net_size"])
        return d
    ...
    def _get_train_anns(self):
        ann_fnames = glob.glob(os.path.join(
            self._train_config["train_annot_folder"], "*.xml"))
        return ann_fnames

    def _get_valid_anns(self):
        ann_fnames = glob.glob(os.path.join(
            self._train_config["valid_annot_folder"], "*.xml"))
        return ann_fnames
