...
class BatchGenerator(object):
    ...
    def _initial_net_size(self):
        self._net_size = DOWNSAMPLE_RATIO * \
            ((self.min_net_size/DOWNSAMPLE_RATIO +
              self.max_net_size/DOWNSAMPLE_RATIO) // 2)
    ...
    def _get(self):
        ...
        for original_box, label in zip(boxes_, coded_labels):
            max_anchor, scale_index, box_index = _find_match_anchor(
                original_box, self.anchors)

            _coded_box = _encode_box(
                list_ys[scale_index], original_box, max_anchor, net_size, net_size)
